...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " fennec-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  fennec-tx [options] <hex-tx> [commands]  Update hex-encoded fennec transaction\n" +
            "or:     fennec-tx [options] -create [commands]   Create hex-encoded fennec transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded fennec transaction
