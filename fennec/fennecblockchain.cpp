...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s FNNC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	if (nHeight == 1)
		...
		nSubsidy = 0.01 * COIN;
	else if (nHeight >= 102 && nHeight <= 110)
		nSubsidy = 0 * COIN;
	else
		nSubsidy = 6.25 * COIN;
 ...
 *
 * Non-static (and re-declared) in fennec/test/txvalidationcache_tests.cpp
 */
