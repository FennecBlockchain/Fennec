...
const char * const BITCOIN_CONF_FILENAME = "fennec.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "fennecd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //fennec-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Fennec
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Fennec
    // Mac: ~/Library/Application Support/Fennec
    // Unix: ~/.fennec
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Fennec";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Fennec";
#else
    // Unix
    return pathRet / ".fennec";
#endif
#endif
}
