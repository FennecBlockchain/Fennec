...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
 ...
 //   widget->setPlaceholderText(QObject::tr("Enter a Fennec address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no fennec: URI
    if(!uri.isValid() || uri.scheme() != QString("fennec"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("fennec:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openBitcoinConf()
{
    ...
    /* Open fennec.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Fennec.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Fennec (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Fennec (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Fennec*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "fennec.desktop";
    return GetAutostartDir() / strprintf("fennec-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a fennec.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Fennec\n";
        else
            optionFile << strprintf("Name=Fennec (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(CFArrayRef listSnapshot, LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find Fennec app
